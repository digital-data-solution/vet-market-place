...
export const submitVCN = async (req, res) => {
  try {
    ...
    const { vcn, documents, notes } = req.body;
    ...
    // FIX #2: Safely mutate subdocuments without breaking Mongoose change tracking
    if (!user.vetDetails) user.vetDetails = {};
    user.vetDetails.vcnNumber = vcn.trim();
    user.markModified('vetDetails');

    if (!user.vetVerification) user.vetVerification = {};
    user.vetVerification.status = 'pending';
    user.vetVerification.submittedAt = new Date();

    // Parse documents as newline-separated links
    ...
    // Store vet's notes separately from admin notes
    if (notes && notes.trim()) {
      user.vetVerification.vetNotes = notes.trim();
    }
...
export const reviewVet = async (req, res) => {
  try {
    ...
    if (action === 'approve') {
      ...
      // FIX #3: Consistent timestamp naming — use reviewedAt for both branches
      ...
      // FIX #3: Consistent timestamp naming — same reviewedAt field as approve
      user.vetVerification.reviewedAt = new Date();
      user.vetVerification.reviewedBy = adminId;
      if (adminNotes) user.vetVerification.adminNotes = adminNotes;
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        reviewedAt: user.vetVerification?.reviewedAt,
